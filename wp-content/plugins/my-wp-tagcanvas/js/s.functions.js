...
//----- Adding script into page head -----
		jQuery('<script>')
			...
			.text('jQuery(window).on("orientationchange",function(event){ mocasi = window.innerWidth; jQuery("#tag_canvas_<?= $inst_id; ?>").attr({"width":mocasi,"height":mocasi}); jQuery("#canvas_wrap_<?= $inst_id; ?>").css({"width":mocasi,"height":mocasi});});')
			.appendTo('body');
